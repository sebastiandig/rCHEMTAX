...
& a_{new} = \frac {a \times(x_{w} \cdot b^T)}
                {((a \cdot b ) \times x_{sd^2}^{-1}) \cdot b^T + 10^{-100}} 
\text{ ; }
b_{new} = \frac {b \times (a^T \cdot x_w) + b_{0w}} 
                {a^T \cdot ((a \cdot b) \times x_{sd^2}^{-1}) + b \times b_{sd^2}^{-1} + 10^{-100}} \\\\
& \text {where; } \\
& x = \text{pigment concentations for each sample}; \\
& x_{sd^2}^{-1} = \frac {1}{x_{sd}^2} = \text{inverse sqaured standard deviation of x}; \\ 
& x_w = x \times x_{sd^2}^{-1} = \text{weighted } x \text{; } \\
& a = \text{taxa contribution}; \\
& b = \text{pigment ratio matrix};\\ 
& b_{sd^2}^{-1} = \frac {1}{b_{0sd}^2}= \text{inverse sqaured standard deviation of initial b}; \\ 
& b_{0w} = b_0 \times b_{sd^2}^{-1}  = \text{weighted initial b} \\
\end{aligned}
$$
...
& i = \text{sample row}; \\
& x_i = (\frac {x_i} {x_{sd_i}})^2; \\
& b_i = \frac {b^T} {x_{sd_i}}; \\
...
-   Minimize $b$ matrix
    ...
    $$
    \begin{aligned}
    & min||x_{j} - a_{ij} \cdot \hat{b_{ij}}|| \\
    & j = \text{pigment}; \\
    & i = \text{taxa with pigment } j; \\
    & x_{j} = 
    \begin{pmatrix} 
            \frac {x_{j}} {x_{sd_{j}}} \\
            \frac {b_{ij}} {b_{sd_{ij}}}
    \end{pmatrix} ;\\
    & a_{ij} = 
    \begin{pmatrix}  
            \frac {a_{ij}} {x_{sd_{j}}} \\
            diag( b_{sd_{ij}}^{-1} )
    \end{pmatrix} ;\\
    & \hat {b_{ij}} = \text {estimated pigment ratio matrix} \\
    \end{aligned}
    $$
...
|            |       | -   *bootnp*        |            |            |
+------------+-------+---------------------+------------+------------+
| *chemtax   | [*    | -   *permcalc*      | calls data |            |
| brokewest* | *Done | -   file for *s*    | and        |            |
|            | **]{s |     and *scol* as   | pigment    |            |
|            | tyle= |     *               | ratio      |            |
|            | "colo | CHEMTAXBROKEWests*, | matrix,    |            |
|            | r: gr | -   file for *f0*,  | selects    |            |
|            | een"} |     *taxa* and      | pigments   |            |
|            |       |     *fcol* as       | for both   |            |
|            |       |     *               | and        |            |
|            |       | pigment_ratios.csv* | creates    |            |
|            |       |                     | std dev    |            |
|            |       |                     | matrices   |            |
+------------+-------+---------------------+------------+------------+
| *CHEMTAXB  | [**   | -   *matfactuvw*    | TODO: test |            |
| ROKEWestb* | Needs | -   *permcalc*      | matfactuvw |            |
|            | wo    | -   file for *b* as | with this  |            |
|            | rk**] |     *b_             |            |            |
|            | {styl | pigment_ratios.csv* |            |            |
|            | e="co | -   file for *x* as |            |            |
|            | lor:  |     *CHE            |            |            |
|            | red"} | MTAXBROKEWestx.csv* |            |            |
+------------+-------+---------------------+------------+------------+
| *CHEMTAXB  |       |                     | as *.csv*  |            |
| ROKEWests* |       |                     | in         |            |
|            |       |                     | *          |            |
